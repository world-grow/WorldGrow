...
class Gaussian:
    ...
    def transform(self, trans):
        trans = torch.tensor(trans).float().to(self.device)
        rotation = trans[:3, :3]
        translation = trans[None, :3, 3]
        scale = (rotation @ rotation.T)[0, 0].sqrt()
        rotation = rotation / scale

        self.from_xyz(self.get_xyz @ rotation.T * scale + translation)
        self._scaling = self._scaling + torch.log(scale) if scale != 1. else self._scaling
        self.from_rotation(utils3d.torch.matrix_to_quaternion(torch.matmul(rotation[None, :, :], utils3d.torch.quaternion_to_matrix(self.get_rotation))))

    def trim_edge(self, ratio=0.01):
        xyz = self.get_xyz
        center_mask = (xyz[:, 0] > self.aabb[0] + self.aabb[3] * ratio) & \
                      (xyz[:, 0] < self.aabb[0] + self.aabb[3] * (1 - ratio)) & \
                      (xyz[:, 1] > self.aabb[1] + self.aabb[4] * ratio) & \
                      (xyz[:, 1] < self.aabb[1] + self.aabb[4] * (1 - ratio))
        self._xyz = self._xyz[center_mask]
        self._scaling = self._scaling[center_mask]
        self._opacity = self._opacity[center_mask]
        self._rotation = self._rotation[center_mask]
        self._features_dc = self._features_dc[center_mask]
