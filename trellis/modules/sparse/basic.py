...
class SparseTensor:
    ...
    def __getitem__(self, idx):
        if isinstance(idx, tuple):
            if len(idx) != 2:
                raise ValueError(f"Tuple indexing expects 2 elements (batch_idx, feature_idx), but got {len(idx)}")
            batch_idx, feature_idx = idx
        else:
            batch_idx, feature_idx = idx, slice(None)

        if isinstance(batch_idx, int):
            batch_idx = [batch_idx]
        elif isinstance(batch_idx, slice):
            batch_idx = range(*batch_idx.indices(self.shape[0]))
        elif isinstance(batch_idx, torch.Tensor):
            if batch_idx.dtype == torch.bool:
                assert batch_idx.shape == (self.shape[0],), f"Invalid index shape: {batch_idx.shape}"
                batch_idx = batch_idx.nonzero().squeeze(1)
            elif batch_idx.dtype in [torch.int32, torch.int64]:
                assert len(batch_idx.shape) == 1, f"Invalid index shape: {batch_idx.shape}"
            else:
                raise ValueError(f"Unknown index type: {batch_idx.dtype}")
        else:
            raise ValueError(f"Unknown index type: {type(batch_idx)}")

        coords = []
        feats = []
        for new_idx, old_idx in enumerate(batch_idx):
            coords.append(self.coords[self.layout[old_idx]].clone())
            coords[-1][:, 0] = new_idx
            feats.append(self.feats[self.layout[old_idx]][:, feature_idx])
