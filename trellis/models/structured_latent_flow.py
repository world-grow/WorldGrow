...
class SLatFlowModel(nn.Module):
    ...
    def convert_to_inpainting(self) -> None:
        with torch.no_grad():
            input_layer_new = sp.SparseLinear(self.in_channels * 2 + 1, self.model_channels if self.io_block_channels is None else self.io_block_channels[0], device=self.input_layer.weight.device)
            nn.init.zeros_(input_layer_new.weight)
            input_layer_new.weight.data[:, :self.in_channels] = self.input_layer.weight.data.clone()
        self.in_channels = self.in_channels * 2 + 1
        self.input_layer = input_layer_new
